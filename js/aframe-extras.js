...
//[start---20191105-fei0079-remove]//
//////// Fei replace by threejs v97 FBXLoader.js
...
// 			//// add the "LastSaved|ApplicationName" 
// 			if(fbxTree){
// 				if ( fbxTree["FBXHeaderExtension"] ){
// 					if ( fbxTree["FBXHeaderExtension"]["SceneInfo"] ){
// 						if ( fbxTree["FBXHeaderExtension"]["SceneInfo"]["LastSaved|ApplicationName"] ){
// 							// console.log("FBXloader.js: FBXTreeParser: return LA:", fbxTree["FBXHeaderExtension"]["SceneInfo"] );
// 							if ( !sceneGraph["LastSaved|ApplicationName"]){
// 								sceneGraph["LastSaved|ApplicationName"] = fbxTree["FBXHeaderExtension"]["SceneInfo"]["LastSaved|ApplicationName"];
// 							}
// 						}
// 					}
// 				}
// 			}
// 			//// the OriginalUnitScaleFactor part
// 			if(fbxTree){
// 				if ( fbxTree["GlobalSettings"] ){
// 					if ( fbxTree["GlobalSettings"]["OriginalUnitScaleFactor"] ){
// 						if ( fbxTree["GlobalSettings"]["OriginalUnitScaleFactor"]["value"] ){
// 							// console.log("FBXloader.js: FBXTreeParser: return OriginalUnitScaleFactor:", fbxTree["GlobalSettings"]["OriginalUnitScaleFactor"]["value"]  );
// 							if ( !sceneGraph["OriginalUnitScaleFactor"]){
// 								sceneGraph["OriginalUnitScaleFactor"] = fbxTree["GlobalSettings"]["OriginalUnitScaleFactor"]["value"];
// 							}
// 						}
// 					}
// 				}
// 			}
// 			// console.log("FBXloader.js: FBXTreeParser: return sceneGraph", sceneGraph, ", fbxTree=", fbxTree);
...
// 	return FBXLoader;
// }();
//[end---20191105-fei0079-remove]//


//[start-20191105-fei0079-add]//
//////// Fei replace the FBX part
...
//[end---20190110-fei0048-add]//
		...
		createAmbientLight: function () {
          ...
          var color = new THREE.Color( r, g, b );
//[start-20191105-fei0079-remove]//
          ////// the light will overlap in mutiple model.
					// sceneGraph.add( new THREE.AmbientLight( color, 1 ) );
//[end---20191105-fei0079-remove]//
				}
...
//[start-20191105-fei0079-add]//
  ...
  update: function update(prevData) {
    if (!prevData) return;
//[start-20191106-fei0079-debug]//
    // console.log("aframe-extras.js: animation-mixer: update  ");
//[end---20191106-fei0079-debug]//
    var data = this.data;
    ...
    if (this.mixer && !isNaN(dt)){
      ...
      if (this.model.animationSlices  ){
        if (this.model.animationSlices[0].uid){
          ...
          for(let i=1;i<this.model.animationSlices.length;i++){
            if(this.model.animationSlices[0].uid == this.model.animationSlices[i].uid){
              var index = i
            }
          }
          
          if (this.model.animationSlices[index]){

            if(this.model.animationSlices[0].changed){
            ...
            if (this.mixer._actions[0].time > this.model.animationSlices[index].endTime){
              ...
              if(this.model.animationSlices[0].uid == this.model.animationSlices[0].idle){
                this.mixer._actions[0].time = this.model.animationSlices[index].startTime;
              }
              else{
                this.model.animationSlices[0].uid = this.model.animationSlices[0].idle;
                for(let i=1;i<this.model.animationSlices.length;i++){
                  if(this.model.animationSlices[0].uid == this.model.animationSlices[i].uid){
                    index = i
                    this.model.el.setAttribute("animation-mixer", "clip: "+this.model.animationSlices[i].animationName);
                  }
                }
                this.mixer._actions[0].time = this.model.animationSlices[index].startTime;
              }
...
module.exports = AFRAME.registerComponent('fbx-model', {
  ...
  load: function load(model) {
//[start-20191031-fei0078-add]//
    // console.log("aframe-extras.js: fbx-model: load: model=", model );
    ...
    checkFBXApplication(model, scale ); //// 
    model.scale.copy( scale.multiplyScalar( 0.01 ) );

//[end---20191031-fei0078-add]//
    this.model = model;
...
// var LOADER_SRC = '../lib/MIFLY_THREE_GLTFLoader.js';
//[end---20191105-fei0079-mod]//
